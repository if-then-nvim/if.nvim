local M = {}
local api = vim.api
local display_width = api.nvim_strwidth
...
local function measure(comp)
  if comp.type == "ascii" then
    ...
    for _, row in ipairs(content) do
      w = max(w, display_width(row))
    end
    return w, #content
  elseif comp.type == "action" then
    local line1 = comp.icon .. "  " .. comp.label
    local line2 = KEY_ICON .. "  " .. (comp.key or "")
    local w = max(display_width(line1), display_width(line2))
    local h = 2
    if comp.desc then
      local line3 = DESC_ICON .. "  " .. comp.desc
      w = max(w, display_width(line3))
      h = 3
    end
    return w, h
  elseif comp.type == "text" then
    local content = type(comp.content) == "function" and comp.content() or (comp.content or "")
    return display_width(content), 1
  end
  return 0, 0
end

local function ascii_line_hl(row, width, rank, split)
  local pad = max(0, floor((width - display_width(row)) / 2))
  local out = {}
  local cur = nil
  logo.each_cell(row, function(col, byte, ch)
    local hl = logo.cell_hl(col, ch, rank, split)
    if cur and cur.hl == hl then
      cur.byte_end = pad + byte + #ch
      return
    end
    if cur then
    ...
    if hl then
      cur = { hl = hl, byte_start = pad + byte, byte_end = pad + byte + #ch }
    end
  end)
  if cur then
    table.insert(out, cur)
...
local function render_lines(comp, width)
  if comp.type == "ascii" then
    local content = type(comp.content) == "function" and comp.content() or comp.content
    local lines = {}
    local colored = comp.color and comp.color ~= "solid"
    local split = comp.split or { 2, 11 }
    local ranks = colored and logo.ranks(content, split) or nil
    local line_hl = colored and {} or nil
    for i, row in ipairs(content) do
      table.insert(lines, center_text(row, width))
      if line_hl and ranks then
        table.insert(line_hl, ascii_line_hl(row, width, ranks[i], split))
      end
    end
  ...
  elseif comp.type == "action" then
    ...
    local gap = "  "
    local gb = #gap
    local icon_hl = comp.group and ("IfDashIcon" .. comp.group:sub(1, 1):upper() .. comp.group:sub(2)) or "IfDashIcon"
    ...
    table.insert(line_hl, {
      ...
      { hl = "IfDashLabel", byte_start = pb + #comp.icon + gb, byte_end = pb + #comp.icon + gb + #comp.label },
    })
    local key = comp.key or ""
    table.insert(lines, left_text(KEY_ICON .. gap .. key, width))
    table.insert(line_hl, {
      { hl = "IfDashKey", byte_start = pb, byte_end = pb + #KEY_ICON + gb + #key },
    })
    if comp.desc then
      table.insert(lines, left_text(DESC_ICON .. gap .. comp.desc, width))
      table.insert(line_hl, {
        { hl = "IfDashDesc", byte_start = pb, byte_end = pb + #DESC_ICON + gb + #comp.desc },
      })
    end
...
local function compose(config, win_width, win_height)
  ...
  for c = 1, grid_cols do
    content_width = content_width + col_w[c]
  end
  if sep_char then
    content_width = content_width + (grid_cols - 1) * display_width(sep_char)
  end
  ...
  local off_x = max(0, floor((win_width - content_width) / 2))
  local off_y = max(0, floor((win_height - total_h) / 2))
  ...
  for key, reg in pairs(regions) do
    ...
    if comp then
      ...
      if sep_char and (reg.c2 - reg.c1) > 0 then
        cell_w = cell_w + (reg.c2 - reg.c1) * display_width(sep_char)
      end
  ...
  for gr = 1, grid_rows do
    ...
    for dy = 0, rh - 1 do
      ...
      if line_idx >= 1 and line_idx <= win_height then
            ...
            if reg and reg.c1 == c then
              if not first and sep_char then
                if is_content_line then
                  ...
                  table.insert(segments, rep(" ", display_width(sep_char)))
                end
              end
        ...
        if display_width(full_line) < win_width then
          full_line = full_line .. rep(" ", win_width - display_width(full_line))
        end
  ...
  local canvas = {}
  for r = 1, win_height do
    canvas[r] = buf_lines[r] or rep(" ", win_width)
  end
...
function M.open(buf, win, action)
  ...
  local ns = api.nvim_create_namespace "ifdashboard"
  local ns_focus = api.nvim_create_namespace "ifdash_focus"
  local config = require("if.config").dashboard
  local win_height = api.nvim_win_get_height(win)
  local win_width = api.nvim_win_get_width(win)

  vim.g.ifdashboard_buf = buf
  vim.g.ifdashboard_win = win
  ...
  local canvas, hl_ranges, action_positions = compose(config, win_width, win_height)
  ...
  local function find_nearest(row, col, dir_r, dir_c)
    ...
    for _, pos in ipairs(positions) do
      if not (pos.row == row and pos.col == col) then
  ...
  vim.bo[buf].filetype = "ifdashboard"
  vim.g.ifdashboard_displayed = true

  local group = api.nvim_create_augroup("IfDashboardAu", { clear = true })

  api.nvim_create_autocmd("BufWinLeave", {
    group = group,
    buffer = buf,
    callback = function()
      vim.g.ifdashboard_displayed = false
      dash_state[buf] = nil
      vim.wo.signcolumn = "yes"
      pcall(api.nvim_del_augroup_by_name, "IfDashboardAu")
    end,
  })
  ...
  api.nvim_create_autocmd({ "WinResized", "VimResized" }, {
    group = group,
    callback = function()
      local dash_buf = vim.g.ifdashboard_buf
      if not dash_buf or not api.nvim_buf_is_valid(dash_buf) then
        return
      end
      M.open(vim.g.ifdashboard_buf, vim.g.ifdashboard_win, "redraw")
    end,
  })
...
function M.setup()
  vim.api.nvim_create_user_command("IfDashboard", function()
    local dash_buf = vim.g.ifdashboard_buf
    if vim.g.ifdashboard_displayed and dash_buf and api.nvim_buf_is_valid(dash_buf) then
      api.nvim_buf_delete(dash_buf, { force = true })
    else
      M.open()
    end
  end, { desc = "Toggle the if.nvim dashboard" })
  ...
  local function try_open()
    for _, w in ipairs(api.nvim_list_wins()) do
      if api.nvim_win_is_valid(w) and api.nvim_win_get_config(w).relative == "" then
        ...
        local pb_name = api.nvim_buf_get_name(pb)
        if not vim.bo[pb].modified and (pb_name == "" or vim.fn.isdirectory(pb_name) == 1) then
          M.open(nil, w)
          if api.nvim_buf_is_valid(pb) and pb ~= vim.g.ifdashboard_buf then
            api.nvim_buf_delete(pb, { force = true })
          end
        end
        return
      end
