local file = {
  ...
  tsx = { icon = "", name = "TypescriptReact", color = "#0088d1" },
  ...
  ["spec.tsx"] = { icon = "", name = "TypescriptReactSpec", color = "#3178C6" },
  ["spec.ts"] = { icon = "", name = "TypescriptSpec", color = "#3178C6" },
  ["test.tsx"] = { icon = "", name = "TypescriptReactTest", color = "#3178C6" },
  ["test.ts"] = { icon = "", name = "TypescriptTest", color = "#3178C6" },
...
local patterns = {
  { "%.spec%.tsx$", file["spec.tsx"], "spec.tsx" },
  { "%.spec%.ts$", file["spec.ts"], "spec.ts" },
  { "%.test%.tsx$", file["test.tsx"], "test.tsx" },
  { "%.test%.ts$", file["test.ts"], "test.ts" },
  { "%.stories%.tsx$", file["stories.tsx"], "stories.tsx" },
  { "%.stories%.ts$", file["stories.ts"], "stories.ts" },
  { "%.stroy%.tsx$", file["stroy.tsx"], "stroy.tsx" },
  { "%.stroy%.ts$", file["stroy.ts"], "stroy.ts" },
}
...
return {
  ...
  config = function()
    ...
    local function match_pattern(name)
      if not name then
        return nil
      end
      for _, p in ipairs(patterns) do
        if name:find(p[1]) then
          return p
        end
      end
      return nil
    ...
    devicons.get_icon = function(name, ext, opts)
      ...
      local p = match_pattern(name)
      if p then
        return orig(p[3], nil, opts)
      end
